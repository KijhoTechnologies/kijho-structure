...
BEGIN
	...
	IF fecha IS NULL THEN
		SET var_salidasRemTotal = (SELECT IF(rd.remd_cantidad IS NOT NULL, SUM(rd.remd_cantidad*-1), 0) FROM remisiones_detalle rd, remisiones r WHERE rd.prod_codigo = codigo AND rd.rem_codigo = r.rem_codigo);
	ELSE
		SET var_salidasRemTotal = (SELECT IF(rd.remd_cantidad IS NOT NULL, SUM(rd.remd_cantidad*-1), 0) FROM remisiones_detalle rd, remisiones r WHERE rd.prod_codigo = codigo AND rd.remd_fecha <= fecha AND rd.rem_codigo = r.rem_codigo);
	END IF;

	SET var_existencias = var_entradasTotal + var_salidasTotal  + var_salidasFFVTotal + var_entradasProRecTotal + var_salidasTrasTotal + var_entradasTrasTotal + var_entradasDevTotal + var_salidasDevTotal + var_salidasRegTotal + var_entradasAjInvTotal + var_salidasPrestTotal + var_entradasPrestTotal + var_salidasCuadTotal + var_salidasRemTotal;

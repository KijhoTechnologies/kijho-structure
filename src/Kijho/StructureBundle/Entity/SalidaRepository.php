...
//            $where_product .= ' AND pid IN (SELECT p1id FROM Kijho\Seitem\BackendBundle\Entity\Producto p1 JOIN Categoria c1 WHERE c1id = :categoryId)';
            $where_product .= ' AND pid IN (SELECT p1id FROM Kijho\Seitem\BackendBundle\Entity\Producto p1 WHERE p1.category = :categoryId)';
        }

//        $dql = "SELECT sid, s.salFecha, s.salCantidad, s.salTotal, pid prod_id, p.category FROM Kijho\Seitem\BackendBundle\Entity\Salida s "
//        $dql = "SELECT sid, s.salFecha, s.salCantidad, s.salTotal, p FROM Kijho\Seitem\BackendBundle\Entity\Salida s "
        $dql = "SELECT s.salFecha, SUM(s.salCantidad) cantidad, SUM(s.salTotal) valor, pid prod_id, p.name product, cid cat_id, c.name category FROM Kijho\Seitem\BackendBundle\Entity\Salida s "
...
//        return $query->getSQL();
    ...
    public function getTotalSalesByProduct($product_id, $date_ini = '', $date_end = '') {
        ...
        $dql = "SELECT SUM(s.salCantidad) cantidad, SUM(s.salTotal) valor "
                . "FROM Kijho\Seitem\BackendBundle\Entity\Salida s JOIN s.prodCodigo p "
...
//        \Doctrine\Common\Util\Debug::dump($query->getResult(), 3); die();
    ...
    public function getSalidaDev($factura, $producto) {
        ...
        $dql = "SELECT * FROM Kijho\Seitem\BackendBundle\Entity\Salida AS sal WHERE sal.facvCodigo = :factura AND sal.prodCodigo = :producto ";
    ...
    public function getKardex($fechaFin, $prodCodigo) {
        ...
        $dql = "SELECT sal.id AS codigoRegistro, "
                ...
                . "FROM Kijho\Seitem\BackendBundle\Entity\Salida AS sal "
                . "JOIN Kijho\Seitem\BackendBundle\Entity\FacturaVentas AS fv WITH sal.facvCodigo = fv.id "
                . "JOIN Kijho\Seitem\BackendBundle\Entity\Cliente AS cli WITH cli.id = fv.id "

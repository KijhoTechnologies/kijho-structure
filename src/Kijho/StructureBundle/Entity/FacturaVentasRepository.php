...
class FacturaVentasRepository extends EntityRepository {
    ...
    public function findFacturasSaldadas($fac) {
        ...
        $dql = "SELECT facvCodigo, facvTotal, c.nombreEmpresa"
                . "FROM Kijho\Seitem\BackendBundle\Entity\FacturaVentas fac "
                . "JOIN  Kijho\Seitem\BackendBundle\Entity\fac.cliCodigo c "
                . "WHERE " . $text
...
//            $where_ruta = " AND idRuta = $ruta  ";
        ...
        $sql = "SELECT tf.idRuta, COUNT(tf.facv_codigo) as cantidad, SUM(tf.facv_total) AS valor, SUM(tf.total_peso) AS peso, tf.facv_fecha FROM "
                ...
                . "FROM Kijho\Seitem\BackendBundle\Entity\factura_ventas  "
                . "JOIN  Kijho\Seitem\BackendBundle\Entity\salida ON factura_ventas.facv_codigo = salida.facv_codigo "
                . "JOIN  Kijho\Seitem\BackendBundle\Entity\producto ON salida.prod_codigo = producto.prod_codigo "
                . "JOIN  Kijho\Seitem\BackendBundle\Entity\cliente ON factura_ventas.cli_codigo = cliente.cli_codigo "
                . "JOIN  Kijho\Seitem\BackendBundle\Entity\zona ON zonaid = cliente.cli_zona_id "
                . "JOIN  Kijho\Seitem\BackendBundle\Entity\ruta ON rutaid = zona.ruta "
    ...
    public function getTotalVentasByUsuario($usuario, $product_id, $date_ini = '', $date_end = '') {
        ...
        $dql = "SELECT COUNT(fac.$usuario) cantidad, SUM(fac.facvTotal) valor "
                . "FROM Kijho\Seitem\BackendBundle\Entity\FacturaVentas fac JOIN fac." . $usuario . " p "
...
//                    return "llgea";
        ...
        $dql = 'SELECT fv.facvCodigo,usu.usuNombre,v.vendNombre,prodid,prod.name,s.salFecha,s.salHora,s.salCantidad,c.nombreEmpresa,c.direccion '
                . 'FROM Kijho\Seitem\BackendBundle\Entity\Salida s '
...
//        $query->setParameter('fechaFin', new DateTime($fechaFin));
    ...
    public function getFacturasVentaCredito($cliente, $barrio_id, $zona_id, $mun_nombre, $dep_nombre, $ruta_id, $array_zonas, $fecha_inicio = '', $fecha_fin = '', $tipo_vendedor, $vendedor_id, $state = 0, $fpcRestaFiltro = 0) {
        ...
        if ($fpcRestaFiltro) {
            $where_fpcRestaFiltro = " AND (fpc.fpcResta > 0 OR ((SELECT count(it) FROM Kijho\Seitem\BackendBundle\Entity\FacturaVentas fv3 JOIN fv3.items it WHERE fv3.facvCodigo = fv.facvCodigo GROUP BY fv3.facvCodigo ) = (SELECT count(it2) FROM Kijho\Seitem\BackendBundle\Entity\Salida it2 WHERE it2.facvCodigo = fv.facvCodigo AND it2.devolucion = 1 )))";
        }
...
//                    return "llgea";
        ...
        $dql = "SELECT fv, c, fpc FROM Kijho\Seitem\BackendBundle\Entity\FacturaVentas fv "
                . "JOIN fv.cliCodigo c "
...
//        \Symfony\Component\VarDumper\VarDumper::dump($query->getResult());die;
    ...
    public function getVentasInDate($fechaInicio, $fechaFin) {
        ...
        $dql = 'SELECT DISTINCT c.nombreEmpresa ,cid, c.identificacion, c.deleted, c.ciudad, '
                ...
                . 'FROM Kijho\Seitem\BackendBundle\Entity\FacturaVentas fv '
                . 'LEFT JOIN fv.facvVendedor v '
    ...
    public function getLastDate($idClient) {
        $dql = 'SELECT fv.facvFecha FROM Kijho\Seitem\BackendBundle\Entity\FacturaVentas fv WHERE fv.cliCodigo = :id ORDER BY fv.facvFecha DESC';
